        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/70 backdrop-blur rounded-full border border-gold-300/50 mb-8">
              ...
              <span className="text-sm font-medium text-coffee-700">{branding.tagline}</span>
            </div>

            <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl font-bold text-coffee-900 leading-tight">
              {branding.content.heroTitle1}
              <span className="block gold-shimmer">{branding.content.heroTitle2}</span>
            </h1>
          ...
          >
            {[
              { icon: Truck, label: branding.content.feature1Title, sub: branding.content.feature1Subtitle },
              { icon: Shield, label: branding.content.feature2Title, sub: branding.content.feature2Subtitle },
              { icon: Star, label: branding.content.feature3Title, sub: branding.content.feature3Subtitle }
            ].map((item, i) => (
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-gold-600 font-medium text-sm uppercase tracking-wider">{branding.content.featuredLabel}</span>
            <h2 className="mt-2 font-display text-4xl font-bold text-coffee-900">{branding.content.featuredTitle}</h2>
      ...
      <section className="py-20 bg-coffee-900 relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          <h2 className="font-display text-3xl sm:text-4xl font-bold text-cream-100">
            {branding.content.ctaTitle1} <span className="text-gold-400">{branding.content.ctaTitle2}</span> {branding.content.ctaTitle3}
          </h2>
          <p className="mt-4 text-cream-300 text-lg">
            {branding.content.ctaSubtitle}
          </p>
          <Link to="/shop" className="mt-8 inline-flex items-center gap-2 btn-gold">
            {branding.content.ctaButtonText}
            <ArrowRight size={18} />
          </Link>
        </div>
      </section>
